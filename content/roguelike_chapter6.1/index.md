+++
title = "roguelike_chapter6.1 schedule和states"
date = 2024-03-30
...
bevy 是一个 ecs 架构的游戏的引擎。schedule 和 states 是系统（s）调度的最小单元。bevy 中系统并不是同时运行的，只有在同一个 schedule 中的系统且没有排序的系统才会并行。
在默认的调度器（MainScheduleOrder）中提供了许多默认的 schedule,它们有着明确的先后顺序。如下:
...
这里面比较重要的两个 schedule 是 StateTransition 和 FixeUpdate，这个是不太常用，属于进阶的内容。FixeUpdate 是根据帧率运行的 schedule，StateTransition 是用于状态转换的 stage。
...
这个系统的主要功能是可以批量的自定义 schedule，每一个 state 会定义三个自定义 schedule，每一个，这个极大的减轻我们的工作量。实现这个功能的代码如下:
...
states 的每一个状态会添加三个 schedule，onEnter，onExit,OnTransition 分别是三个状态，进入时，离开时，和状态转换。StateTransitionEvent 则是状态转换的事件。
